...
#this is a lot of yaml files, question is do we want users to download all of these resources or make them available in the script as is?
function setupStatefulset() {
    oc project $TO_NAMESPACE
    #apply all prereq resources (currently in statefulset-resources dir)
...
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/managed-by: mongodbs.operator.ibm.com
    app.kubernetes.io/name: mongodbs.operator.ibm.com
    certmanager.k8s.io/time-restarted: 2023-3-22.0857
    release: mongodb
spec:
  replicas: 1
  selector:
  ...
  revisionHistoryLimit: 10
EOF
    #wait for statefulset icp-mongodb to become ready
    check_healthy $TO_NAMESPACE
...
function check_healthy() {
    ...
    sleep 10
    pod=$(oc get pods -n ${CS_NAMESPACE} | grep icp-mongodb-0 | awk '{print $1}')
    
    while true; do
        if [[ ${retries} -eq 0 ]]; then
            error "Timeout after ${total_time_mins} minutes waiting for MongoDB to deploy"
        fi
        ...
        if [[ "${phase}" != "Running" ]]; then
            retries=$(( retries - 1 ))
            info "RETRYING: Waiting for MongoDB Statefulset is Succeeded (${retries} left)"
            sleep ${sleep_time}
        else
            msg "-----------------------------------------------------------------------"    
            success "MongoDB setup is deployed in ${CS_NAMESPACE}"
            break
        fi
    done
}
