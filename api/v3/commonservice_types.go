...
// AppliedCommonServiceSpec captures ALL spec fields that were applied
type AppliedCommonServiceSpec struct {
	// Features configuration
	Features *AppliedFeatures `json:"features,omitempty"`

	// Installation and approval settings
	...
	HugePages       *AppliedHugePages       `json:"hugepages,omitempty"`
	OperatorConfigs []AppliedOperatorConfig `json:"operatorConfigs,omitempty"`
	...
	// Custom service configurations (count only, not full details)
	ServicesCount int `json:"servicesCount,omitempty"`
}

// AppliedFeatures captures the features configuration
type AppliedFeatures struct {
	Bedrockshim *AppliedBedrockshim `json:"bedrockshim,omitempty"`
	APICatalog  *AppliedAPICatalog  `json:"apiCatalog,omitempty"`
}

// AppliedBedrockshim captures bedrockshim configuration
type AppliedBedrockshim struct {
	Enabled                   bool `json:"enabled,omitempty"`
	CrossplaneProviderRemoval bool `json:"crossplaneProviderRemoval,omitempty"`
}

// AppliedAPICatalog captures API catalog configuration
type AppliedAPICatalog struct {
	StorageClass string `json:"storageClass,omitempty"`
}

// AppliedHugePages captures hugepages configuration
type AppliedHugePages struct {
	Enable         bool              `json:"enable,omitempty"`
	HugePagesSizes map[string]string `json:"hugePagesSizes,omitempty"`
}

// AppliedOperatorConfig captures operator configuration
type AppliedOperatorConfig struct {
	Name        string `json:"name,omitempty"`
	Replicas    *int32 `json:"replicas,omitempty"`
	UserManaged bool   `json:"userManaged,omitempty"`
}

// AppliedLicense captures license configuration
...
// MergedServiceSummary provides a summary of a service in the merged OperandConfig
type MergedServiceSummary struct {
	Name               string `json:"name"`
	ManagementStrategy string `json:"managementStrategy,omitempty"`
	ResourceCount      int    `json:"resourceCount,omitempty"`
}

// SourceCR identifies a CommonService CR that contributed to the merge
type SourceCR struct {
	Namespace string `json:"namespace"`
	Name      string `json:"name"`
	Priority  int    `json:"priority"`
}

// MergeConflict documents a conflict that occurred during merge
type MergeConflict struct {
	Field      string   `json:"field"`
	Values     []string `json:"values"`
	Resolution string   `json:"resolution"`
	Winner     string   `json:"winner"` // namespace/name of winning CR
}

// MergeInfo tracks information about the merge process
type MergeInfo struct {
	SourceCRCount int             `json:"sourceCRCount"`
	SourceCRs     []SourceCR      `json:"sourceCRs,omitempty"`
	Conflicts     []MergeConflict `json:"conflicts,omitempty"`
	LastMergeTime string          `json:"lastMergeTime,omitempty"`
}
...
type CommonServiceStatus struct {
	...
	// AppliedSpec contains the complete CommonService spec that was used to generate the current configuration
	// +optional
	AppliedSpec AppliedCommonServiceSpec `json:"appliedSpec,omitempty"`
	// MergedServices contains a summary of services from the merged OperandConfig
	// +optional
	MergedServices []MergedServiceSummary `json:"mergedServices,omitempty"`
	// MergeInfo tracks information about the merge process across multiple CommonService CRs
	// +optional
...
// UpdateMergedConfigStatus updates the status with complete merged configuration information
func (r *CommonService) UpdateMergedConfigStatus(
	mergedCR *CommonService,
	conflicts []MergeConflict,
	sourceCRs []SourceCR,
	mergedServices []MergedServiceSummary,
) {
	// Capture ALL spec fields from the merged CR
	r.Status.AppliedSpec = AppliedCommonServiceSpec{
		// Features
		Features: convertFeatures(mergedCR.Spec.Features),

		// Installation and approval
		InstallPlanApproval: string(mergedCR.Spec.InstallPlanApproval),
		ManualManagement:    mergedCR.Spec.ManualManagement,

		// Security
		FipsEnabled:               mergedCR.Spec.FipsEnabled,
		BYOCACertificate:          mergedCR.Spec.BYOCACertificate,
		DisableManageCertRotation: mergedCR.Spec.DisableManageCertRotation,

		// Networking
		RouteHost: mergedCR.Spec.RouteHost,
		...
		// Storage
		StorageClass: mergedCR.Spec.StorageClass,

		// Namespaces (use status values as they may differ from spec)
		ServicesNamespace: r.Status.ConfigStatus.ServicesNamespace,
		OperatorNamespace: r.Status.ConfigStatus.OperatorNamespace,

		// Catalog
		CatalogName:      r.Status.ConfigStatus.CatalogName,
		CatalogNamespace: r.Status.ConfigStatus.CatalogNamespace,

		// User
		DefaultAdminUser: mergedCR.Spec.DefaultAdminUser,

		// Labels
		Labels: mergedCR.Spec.Labels,

		// Advanced
		HugePages:       convertHugePages(mergedCR.Spec.HugePages),
		OperatorConfigs: convertOperatorConfigs(mergedCR.Spec.OperatorConfigs),

		// License
		License: AppliedLicense{
			Accept:  mergedCR.Spec.License.Accept,
			Use:     mergedCR.Spec.License.Use,
			License: mergedCR.Spec.License.License,
			Key:     mergedCR.Spec.License.Key,
		},

		// Monitoring
		EnableInstanaMetricCollection: mergedCR.Spec.EnableInstanaMetricCollection,

		// Image
		ImagePullSecret: mergedCR.GetImagePullSecret(),

		// Services count
		ServicesCount: len(mergedCR.Spec.Services),
	}

	// Set merged services
	r.Status.MergedServices = mergedServices

	// Set merge info
	r.Status.MergeInfo = MergeInfo{
		SourceCRCount: len(sourceCRs),
		SourceCRs:     sourceCRs,
		Conflicts:     conflicts,
		LastMergeTime: time.Now().Format(time.RFC3339),
	}
}

// convertFeatures converts Features to AppliedFeatures
func convertFeatures(f *Features) *AppliedFeatures {
	if f == nil {
		return nil
	}
	result := &AppliedFeatures{}
	if f.Bedrockshim != nil {
		result.Bedrockshim = &AppliedBedrockshim{
			Enabled:                   f.Bedrockshim.Enabled,
			CrossplaneProviderRemoval: f.Bedrockshim.CrossplaneProviderRemoval,
		}
	}
	if f.APICatalog != nil {
		result.APICatalog = &AppliedAPICatalog{
			StorageClass: f.APICatalog.StorageClass,
		}
	}
	return result
}

// convertHugePages converts HugePages to AppliedHugePages
func convertHugePages(hp *HugePages) *AppliedHugePages {
	if hp == nil {
		return nil
	}
	return &AppliedHugePages{
		Enable:         hp.Enable,
		HugePagesSizes: hp.HugePagesSizes,
	}
}

// convertOperatorConfigs converts OperatorConfig slice to AppliedOperatorConfig slice
func convertOperatorConfigs(configs []OperatorConfig) []AppliedOperatorConfig {
	if configs == nil {
		return nil
	}
	result := make([]AppliedOperatorConfig, len(configs))
	for i, cfg := range configs {
		result[i] = AppliedOperatorConfig{
			Name:        cfg.Name,
			Replicas:    cfg.Replicas,
			UserManaged: cfg.UserManaged,
		}
	}
	return result
}
