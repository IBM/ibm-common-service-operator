...
function check_sub_version(){
    ...
    elif [[ "$iam_operator_channel" == "v3" ]] && [[ "$odlm_channel" == "v3" ]] && [[ "$cs_operator_channel" == "v3" ]] && [[ "$cert_manager_channel" == "v3" ]]; then
        success " cert-manager, IAM, ODLM and CS-Operator subscriptions are all in correct channel v3 for LTSR"
        append_check "eus_installer" "check subscription channel" "ok" "cert-manager, IAM, ODLM and CS-Operator subscriptions are all in correct channel v3 for LTSR" ""
    elif [[ "$iam_operator_channel" == "v3.23" ]] && [[ "$odlm_channel" == "v3.23" ]] && [[ "$cs_operator_channel" == "v3.23" ]] && [[ "$cert_manager_channel" == "v3.23" ]]; then
        success " cert-manager, IAM, ODLM and CS-Operator subscriptions are all in correct channel v3 for LTSR"
        append_check "eus_installer" "check subscription channel" "ok" "cert-manager, IAM, ODLM and CS-Operator subscriptions are all in correct channel v3.23 for LTSR" ""
    else
