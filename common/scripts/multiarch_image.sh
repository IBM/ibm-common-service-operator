...
echo "Creating the multi-arch image manifest for ${IMAGE_REPO}/${IMAGE_NAME}:3.3.0..."
${CONTAINER_CLI} manifest create "${IMAGE_REPO}"/"${IMAGE_NAME}":3.3.0 \
    "${IMAGE_REPO}"/"${IMAGE_NAME}"-amd64:"${VERSION}" \
    "${IMAGE_REPO}"/"${IMAGE_NAME}"-ppc64le:"${VERSION}" \
    "${IMAGE_REPO}"/"${IMAGE_NAME}"-s390x:"${VERSION}"
...
echo "Pushing the multi-arch image manifest for ${IMAGE_REPO}/${IMAGE_NAME}:3.3.0..."
${CONTAINER_CLI} manifest push "${IMAGE_REPO}"/"${IMAGE_NAME}":3.3.0
