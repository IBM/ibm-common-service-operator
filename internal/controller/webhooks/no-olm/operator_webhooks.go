...
func (webhookConfig *CSWebhookConfig) SetupServer(mgr manager.Manager, namespace string) error {
	...
	if err := webhookConfig.ReconcileService(context.TODO(), client, namespace); err != nil {
		return err
	}
...
func (webhookConfig *CSWebhookConfig) Reconcile(ctx context.Context, client client.Client) error {
	...
	if err := webhookConfig.ReconcileService(ctx, client, namespace); err != nil {
		return err
	}
...
func (webhookConfig *CSWebhookConfig) ReconcileService(ctx context.Context, client client.Client, namespace string) error {
...
func createService(ctx context.Context, client client.Client, namespace string) error {
...
func (webhookConfig *CSWebhookConfig) setupCerts(ctx context.Context, client client.Client, namespace string) error {
	// Wait for the secret to te created
	secret := &corev1.Secret{}
	err := wait.PollImmediate(time.Second*1, time.Second*30, func() (bool, error) {
		err := client.Get(ctx,
			types.NamespacedName{Name: caCertSecretName, Namespace: namespace},
			secret)
		if err != nil {
			if errors.IsNotFound(err) {
				return false, nil
...
func (webhookConfig *CSWebhookConfig) waitForCAInConfigMap(ctx context.Context, client client.Client, namespace string) ([]byte, error) {
	...
	err := wait.PollImmediate(time.Second, time.Second*30, func() (bool, error) {
		...
		if err := client.Get(ctx,
			types.NamespacedName{Name: webhookConfig.CAConfigMap, Namespace: namespace},
			caConfigMap,
		); err != nil {
			if errors.IsNotFound(err) {
				return false, nil
